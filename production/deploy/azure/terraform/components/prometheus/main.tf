...
# These resources were largely derived from https://github.com/Azure/prometheus-collector/blob/main/AddonTerraformTemplate/main.tf
resource "azurerm_monitor_workspace" "prometheus" {
  name                = "${var.operator}-${var.environment}-${var.region}-monitor-workspace"
...
resource "azurerm_monitor_data_collection_endpoint" "data_collection_endpoint" {
  name                = substr("MSProm-${var.region}-${var.aks_cluster_name}", 0, min(44, length("MSProm-${var.region}-${var.aks_cluster_name}")))
  resource_group_name = var.resource_group_name
  location            = var.region
  kind                = "Linux"
}
...
resource "azurerm_monitor_data_collection_rule" "data_collection_rule" {
  name                        = substr("MSProm-${var.region}-${var.aks_cluster_name}", 0, min(64, length("MSProm-${var.region}-${var.aks_cluster_name}")))
  resource_group_name         = var.resource_group_name
  location                    = var.region
  ...
  destinations {
    monitor_account {
      ...
      name               = "monitoring-account-${var.environment}"
    }
  }

  data_flow {
    streams      = ["Microsoft-PrometheusMetrics"]
    destinations = ["monitoring-account-${var.environment}"]
  }
...
resource "azurerm_monitor_data_collection_rule_association" "data_collection_rule_association" {
  name                    = "MSProm-${var.region}-${var.aks_cluster_name}"
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  ...
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.prometheus.id, var.aks_cluster_id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "ux_recording_rules_rule_group_windows" {
  name                = "UXRecordingRulesRuleGroup-Win - ${var.aks_cluster_name}"
  location            = var.region
  resource_group_name = var.resource_group_name
  cluster_name        = var.aks_cluster_name
