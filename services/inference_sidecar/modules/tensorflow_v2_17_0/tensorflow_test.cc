...
TEST(TensorflowCancellationTest, Failure_RegisterModelWithCancelledContext) {
  ...
  absl::StatusOr<RegisterModelResponse> status =
      tensorflow_module->RegisterModel(register_request, cancelled_context);
  ASSERT_FALSE(status.ok());
  EXPECT_EQ(status.status().code(), absl::StatusCode::kCancelled);
}

TEST(TensorflowCancellationTest, Failure_DeleteModelWithCancelledContext) {
  InferenceSidecarRuntimeConfig config;
  std::unique_ptr<ModuleInterface> tensorflow_module =
      ModuleInterface::Create(config);
  ...
  absl::StatusOr<DeleteModelResponse> status =
      tensorflow_module->DeleteModel(delete_request, cancelled_context);
  ASSERT_FALSE(status.ok());
  EXPECT_EQ(status.status().code(), absl::StatusCode::kCancelled);
}

TEST(TensorflowCancellationTest, Failure_PredictWithCancelledContext) {
  InferenceSidecarRuntimeConfig config;
  std::unique_ptr<ModuleInterface> tensorflow_module =
      ModuleInterface::Create(config);
  PredictRequest predict_request;
  MockCancellableContext cancelled_context{true};
  absl::StatusOr<PredictResponse> status = tensorflow_module->Predict(
...
TEST(TensorflowModuleTest, Success_PredictEmbed) {
  ...
  EXPECT_TRUE(absl::StrContains(
      ...
      "\"data_type\":\"FLOAT\",\"tensor_content\":[0.7276110649108887,0."
      "0728105753660202,0.11053492873907089,0.6876803636550903,0."
